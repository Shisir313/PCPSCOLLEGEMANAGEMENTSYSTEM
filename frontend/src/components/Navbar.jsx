...
export default function Navbar() {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'shadow-xl' : 'shadow-lg'}`}>
      ...
      {/* ── Top red banner ── */}
      <div className="bg-pcps-red overflow-hidden relative">
        <div className="absolute inset-0 bg-gradient-to-r from-pcps-red-dark via-pcps-red to-pcps-red-dark opacity-60" />
        <div className="relative max-w-7xl mx-auto px-4 py-1.5 flex items-center justify-between">
          ...
          {user && (
            <p className="text-white/80 text-xs hidden sm:flex items-center gap-1.5">
              <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z"/>
              </svg>
              <span className="font-semibold text-white">{user.username}</span>
              <span className="capitalize bg-white/20 text-white text-[10px] px-2 py-0.5 rounded-full">
                {user.role}
              </span>
            </p>
          )}
